...
| expB_1 (baseline) | 0.0001 | 0.99 | 32 | 1.0 | 0.05 | 0.1 | Reward stayed flat near -20.8 (last 20 episodes); establishes the group's baseline |
...
| expB_3 (large batch) | 0.0001 | 0.99 | 64 | 1.0 | 0.05 | 0.1 | **Best result of all 30 experiments** — reached -7.75 mean reward (last 20 episodes), including a best single-episode result of +8 (an outright win, roughly 21–13 against the built-in opponent) |
| expB_4 (extra-large batch) | 0.0001 | 0.99 | 128 | 1.0 | 0.05 | 0.1 | Reward stayed near -20.9, flat like baseline. An earlier training run at this configuration showed a promising trend before compute-quota interruptions across platforms forced a rerun; the rerun did not reproduce the earlier result, illustrating the run-to-run variance inherent to DQN (see Limitations) |
| expB_5 (low epsilon floor) | 0.0001 | 0.99 | 32 | 1.0 | 0.01 | 0.1 | Modest improvement to -18.8; near-fully-greedy behavior helped exploit learned policy slightly |
...
| expB_7 (fast epsilon decay) | 0.0001 | 0.99 | 32 | 1.0 | 0.05 | 0.02 | Strong improvement to -12.45, with a best single episode of -5.0; committing to exploitation early helped substantially |
...
1. **Batch size was the single most impactful hyperparameter** across all 30 experiments. Increasing batch size from 32 to 64 (Henriette, expB_3) improved mean reward from ~-20.8 to **-7.75**, the best average result the group achieved.
2. **Two separate configurations produced outright wins.** Both `expB_3` (+8, ~21–13) and `expB_10` (+2, ~21–19) — the group's two strongest average performers — each recorded at least one episode where the agent genuinely beat the built-in opponent, not just "lost by less." Both winning configs shared `batch_size=64`, reinforcing batch size as the dominant factor in the group's results.
3. **Training duration matters as much as hyperparameter choice.** The clearest learning signals in the group — Henriette's batch/epsilon experiments and Patricia's C07 — were also among the longer-trained or more favorably-configured runs, while most 50k–150k-timestep runs stayed flat near -21 regardless of configuration.
4. **Learning rate and gamma had minimal standalone effect** within the timestep budgets tested; extreme values (very high or very low lr) showed no divergence but also no learning.
5. **Epsilon schedule had a real effect on top of batch size.** A fast decay to a low floor (expB_7, expB_8, expB_10) gave several of the group's best results, while too-high an exploration floor (0.2) or too little initial exploration (start=0.5) consistently hurt performance.
6. **Best overall configuration:** `batch_size=64, lr=0.0001, gamma=0.99, eps_start=1.0, eps_end=0.05, eps_decay_frac=0.1`, trained for 500,000 timesteps (Henriette Utatsineza, expB_3) — final mean reward **-7.75** over the last 20 evaluation episodes, with a best single-episode result of +8 (an outright win).
...
The final `models/dqn_model.zip` was trained using the best combined configuration identified above: `batch_size=64, lr=0.0001, gamma=0.99, eps_start=1.0, eps_end=0.05, eps_decay_frac=0.1`, trained for 500,000 timesteps (matching expB_3, the group's best-performing run).

- **Final training reward (mean, last 20 episodes):** -7.75
- **Best single-episode result:** +8 (an outright win, ~21–13, against the built-in opponent)
- **Evaluation mean reward (5 episodes, greedy policy):** [fill in from `play.py --record` run]
...
=======
>>>>>>> 4725c9aa36a8bdd07b6f741b61a024eed5e905bb
```